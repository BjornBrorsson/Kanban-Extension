...
async function runFailureMatrixTests() {
  ...
  const testRoot = path10.join(__dirname, "..", "scratch", "failure-matrix-workspace");
  if (fs9.existsSync(testRoot)) {
  ...
  const testFile2 = path10.join(testRoot, "target_file2.txt");
  const originalContent2 = "Pristine original content\n";
  fs9.writeFileSync(testFile2, originalContent2, "utf8");
  ...
  assert(fs9.readFileSync(testFile2, "utf8") === originalContent2, "No partial committed state on sudden process crash");
  const recovered = await runtime1.initialize();
  assert(fs9.readFileSync(testFile2, "utf8") === originalContent2, "Target file remains 100% pristine after recovery");
  ...
  const lease3 = runtime1.leaseManager.acquireLease("FAIL-EXPIRE-03", 10);
  ...
  runtime1.budgetManager.registerAccount({ currency: "EUR", totalCeiling: 10, totalSpentReported: 0, totalSpentEstimated: 0, totalReserved: 0, isSubscriptionQuota: false });
  const res6 = runtime1.budgetManager.reserve({ ticketId: "FAIL-CANCEL-06", attemptId: "att-c6", totalAmount: 2 });
  ...
  assert(runtime1.budgetManager.getAccount("EUR").totalReserved === 2, "Budget reservation must NOT be released if termination unconfirmed");
  ...
  const testFile7 = path10.join(testRoot, "file7.txt");
  fs9.writeFileSync(testFile7, "Initial pristine file 7\n", "utf8");
  ...
  const worktreeFile8 = path10.join(alloc8.workspacePath, "diverge.txt");
  fs9.writeFileSync(worktreeFile8, "Agent modifications\n", "utf8");
  fs9.writeFileSync(testFile8, "Human user WIP modifications\n", "utf8");
  ...
  assert(fs9.readFileSync(testFile8, "utf8") === "Human user WIP modifications\n", "Human WIP preserved 100% intact");
...
//# sourceMappingURL=failureMatrixTests.js.map
