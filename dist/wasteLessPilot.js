...
// test/pilots/wasteLessPilot.ts
var wasteLessPilot_exports = {};
__export(wasteLessPilot_exports, {
  runWasteLessPilot: () => runWasteLessPilot
});
module.exports = __toCommonJS(wasteLessPilot_exports);
...
async function runWasteLessPilot() {
  console.log("\n=== Executing Pilot Demonstration 2: WasteLess Android Overnight Run ===");
  const pilotRoot = path10.join(__dirname, "..", "..", "scratch", "wasteless-pilot");
  if (fs9.existsSync(pilotRoot)) {
    fs9.rmSync(pilotRoot, { recursive: true, force: true });
  }
  fs9.mkdirSync(pilotRoot, { recursive: true });
  const dummyMainDart = path10.join(pilotRoot, "main.dart");
  fs9.writeFileSync(dummyMainDart, `void main() { print("WasteLess Android Core"); }
`, "utf8");
  ...
  assert(profile.canDispatchNext(12).allowed === false, "Attempt over \u20AC10.00 ceiling blocked immediately by pre-dispatch gate");
  ...
  for (const cmd of verificationCommands) {
    ...
    const out = cp3.execSync(cmd).toString().trim();
    verificationLogs.push(out);
  }
  const apkDir = path10.join(pilotRoot, "build", "app", "outputs", "flutter-apk");
  fs9.mkdirSync(apkDir, { recursive: true });
  const apkFileName = `app-release-${testedCommitSha.slice(0, 8)}.apk`;
  const apkFilePath = path10.join(apkDir, apkFileName);
  fs9.writeFileSync(apkFilePath, `MOCK ANDROID BINARY - Git SHA: ${testedCommitSha}
`, "utf8");
  const artifactMetaPath = path10.join(apkDir, `${apkFileName}.meta.json`);
  const artifactMeta = {
  ...
  fs9.writeFileSync(artifactMetaPath, JSON.stringify(artifactMeta, null, 2), "utf8");
  const cashSpent = 3.4;
  const subscriptionUnits = 18;
  ...
  const reportOutPath = path10.join(pilotRoot, "overnight_summary.md");
  fs9.writeFileSync(reportOutPath, outcomeMarkdown, "utf8");
...
//# sourceMappingURL=wasteLessPilot.js.map
