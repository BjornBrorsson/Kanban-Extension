...
// test/pilots/selfHostPilot.ts
var selfHostPilot_exports = {};
__export(selfHostPilot_exports, {
  runSelfHostPilot: () => runSelfHostPilot
});
module.exports = __toCommonJS(selfHostPilot_exports);
...
var LeadPlanningEngine = class {
  ...
  static writeSubtasksToBoard(boardBacklogDir, result) {
    if (!fs9.existsSync(boardBacklogDir)) {
      fs9.mkdirSync(boardBacklogDir, { recursive: true });
    }
    const createdFiles = [];
    for (const subtask of result.subtasks) {
      ...
      const filePath = path10.join(boardBacklogDir, filename);
...
async function runSelfHostPilot() {
  console.log("\n=== Executing Pilot Demonstration 1: Agentic Kanban Self-Hosting ===");
  const steps = [];
  const pilotRoot = path11.join(__dirname, "..", "..", "scratch", "self-host-pilot");
  if (fs10.existsSync(pilotRoot)) {
    fs10.rmSync(pilotRoot, { recursive: true, force: true });
  }
  fs10.mkdirSync(pilotRoot, { recursive: true });
  const targetFile = path11.join(pilotRoot, "cardBadges.ts");
  fs10.writeFileSync(targetFile, `export function renderBadge(status: string): string {
  return status;
}
`, "utf8");
...
`;
  ...
  assert(fs10.existsSync(targetFile), "Target file remains 100% intact");
...
//# sourceMappingURL=selfHostPilot.js.map
